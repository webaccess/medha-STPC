...
module.exports = {
  ...
  async validateActivityBatchStudents(ctx) {
    ...
    if (!batch) {
      return ctx.response.badRequest("Activity batch is missing");
    }

    /**
     * Check whether activity batch exist
     */

    const activityBatch = await strapi
      .query("activity-batch")
      .findOne({ id: batch, activity: id });
    ...
    const areStudentPresentInActivityBatch = await Promise.all(
      students.map(studentId =>
        strapi
          .query("activity-batch-attendance")
          .findOne({ activity_batch: batch, student: studentId })
      )
    );
    ...
    return strapi.services.activity.validateStudentForActivityBatch(ctx);
  }
};
